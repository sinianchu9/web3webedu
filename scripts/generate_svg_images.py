...
def update_frontmatter(file_path: Path, cluster: str, slug: str):
    ...
    # 生成图片路径
    image_path = f"/images/{cluster}/{slug}.svg"
    full_image_path = PUBLIC_IMAGES_DIR / f"{cluster}" / f"{slug}.svg"
    ...
    # 生成 SVG
    try:
        generate_svg(title, description, cluster, str(full_image_path))
...
def main():
    print("=" * 60)
    print("edu 项目批量生成 SVG 配图（含关键词 + 域名 + 标题 + 摘要 + 加密货币图案）")
    print("=" * 60)
